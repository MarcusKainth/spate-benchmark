...
|---|---|---|---|
| `parallelism` | **4** | `FLINK_PROPERTIES`, `TASK_MANAGER_NUMBER_OF_TASK_SLOTS` | Subtasks, one per TaskManager CPU. The topic has 8 partitions, so each owns two. `KafkaSource` gives one partition to one subtask, so a ninth would never receive a split. |
...
| `inflight` | **2** | `SINK_MAX_IN_FLIGHT` | Concurrent INSERTs per subtask. The connector's default is **50**, which at parallelism 4 would permit 200 concurrent INSERTs and a corresponding flood of parts. |
...
flight are `parallelism × inflight × max_rows` = **400,000**, and total buffered
rows are `parallelism × buffered_rows` = **400,000**. The arm holds `parallelism`
...
| Key | Value | Why |
|---|---|---|
| `taskmanager.memory.process.size` | `15872m` | Total process memory in the 16 GiB container, 512 MiB slack. `entrants_are_valid` refuses anything below `14336m`, so this is partly a floor the harness imposes. |
...
across two containers: the TaskManager gets the full 4 CPU / 16 GiB data-plane
