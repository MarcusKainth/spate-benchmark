...
/** Source the reader is asking about: what actually renders the page. */
const ROOTS = [path.join(SITE, 'src'), path.join(SITE, 'plugins')];
...
test('the site does not branch on any entrant id', () => {
  ...
  for (const file of ROOTS.flatMap((r) => sourceFiles(r))) {
    ...
    for (const id of ids) {
      ...
      if (line >= 0) {
        offences.push(`${path.relative(REPO, file)}:${line + 1} names the entrant "${id}"`);
      }
    }
  }
