...
fn main() {
    ...
    assert!(events_per_batch > 0, "workload.toml: events_per_batch must be positive");
    ...
    let unit_list = units
        ...
        .collect::<Vec<_>>()
        .join(", ");
    writeln!(out, "// @generated by harness/build.rs from workload/workload.toml — do not edit.").unwrap();
    writeln!(out, "/// Events per message, and therefore the fan-out factor.").unwrap();
    ...
    writeln!(out, "pub const UNITS: [&str; {}] = [{unit_list}];", units.len()).unwrap();
    writeln!(out, "/// The unit value tier B filters out.").unwrap();
    writeln!(out, "pub const DROP_UNIT: &str = {drop_unit:?};").unwrap();
    writeln!(out, "/// Tier B drops rows whose `quality` is non-null and below this.").unwrap();
...
fn workload_dir() -> PathBuf {
    let manifest = PathBuf::from(std::env::var_os("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR is set"));
    let dir = manifest
        .parent()
